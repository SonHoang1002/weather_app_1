...
class _WeatherPageState extends State<WeatherPage> {
  ...
  List<NextTimeModel>? nextTimeModel;
                ...
                child: Column(children: [
                  ...
                  Container(
                    ...
                    child: ListView.builder(
                        ...
                        itemCount: nextTimeModel?.length ?? 7,
                          ...
                          return WeatherTime(
                              hour: index == 0
                                  ...
                                  : (((nextTimeModel![index].dtTxt)!
                                      ...
                                      "0"),
                              temp:
                                  ((nextTimeModel![index].main!.temp! - K_to_C)
                                          .round()) ??
                                      0);
                        })),
                  )
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Container(
                      child: FutureBuilder(
                        future: Future.value(nextDayModel),
                        ...
                        if (snapshot.connectionState == ConnectionState.done) {
                          return Container(
                            ...
                            child: ListView.builder(
                                ...
                                itemCount: nextDayModel?.list?.length ?? 10,
                                itemBuilder: ((context, index) {
                                  return WeatherDay(
                                      day: listOfDay[index],
  ...
  int setMinAndMaxTempature() {
    ...
    if (nextDayModel?.list?.length == 0 || nextDayModel?.list == null) {
      ...
      for (int i = 0; i < (nextDayModel?.list!.length ?? 0); i++) {
        if ((nextDayModel?.list![i].temp!.max!.ceil() ?? 0) > max) {
          max = nextDayModel?.list![i].temp!.max!.ceil() ?? 20;
        }
        if ((nextDayModel?.list![i].temp!.min!.ceil() ?? 0) < min) {
          min = nextDayModel?.list![i].temp!.min!.ceil() ?? 20;
        }
      }
